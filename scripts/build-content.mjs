...
/** Read the standalone about page (content/whoami/index.mdx). */
async function buildAbout() {
  const file = path.join(path.dirname(PROJECTS_DIR), 'whoami', 'index.mdx')
  if (!(await exists(file))) return { title: 'whoami', lead: '', body: '' }
  const { data: fm, content } = matter(await fs.readFile(file, 'utf8'))
  return {
    title: typeof fm.title === 'string' ? fm.title : 'whoami',
    lead: typeof fm.lead === 'string' ? fm.lead : '',
    body: content.trim(),
  }
}
...
async function main() {
  ...
  await emitManifest(projects, await buildAbout())
...
async function emitManifest(projects, about) {
